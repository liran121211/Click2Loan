...
public class ClientLoansManager {
    ...
    public ClientLoansManager(Scene scene) {
        this.scene = scene;
    ...
    public void initializeScreen() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("clientLoans.fxml"));
            scene.setRoot(loader.load());
            scene.getWindow().setWidth(WINDOW_WIDTH);
            scene.getWindow().setHeight(WINDOW_HEIGHT);
            scene.setUserData(loader);

            ClientLoansController controller = loader.getController();
    ...
    protected void inspectLoan() {
        ...
        inspectLoanManager.initializeScreen(((ClientLoansController) ((FXMLLoader) scene.getUserData()).getController()).getLoans_list().getSelectionModel().getSelectedItem().getId());
    }
